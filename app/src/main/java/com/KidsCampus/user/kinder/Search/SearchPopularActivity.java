...
public class SearchPopularActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_search_popular);

        search = findViewById(R.id.searchpopular_search);
        recyclerView = findViewById(R.id.searchpopular_recyclerview);
        searchtext = findViewById(R.id.searchpopular_edittext);
        init = findViewById(R.id.searchpopular_init);
        nodata = findViewById(R.id.searchpopular_nodata);
        waiting = findViewById(R.id.searchpopular_waiting);
        progressBar = findViewById(R.id.searchpopular_progrssbar);
        ...
        search.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(searchtext.getText().toString().isEmpty() || searchtext.getText().toString().length() < 2) {
                    Toast.makeText(SearchPopularActivity.this, "검색어는 두 글자 이상 입력해주세요", Toast.LENGTH_SHORT).show();
                } else {
                    search_data.clear();
                    checkfirst = true;
                    ...
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            Query query = db.collection("실시간인기글").orderBy("write_time", Query.Direction.DESCENDING).limit(20);
                            getInfo(query);
                        }
                    }, 500);
                    searchtext.setText("");
                    ...
                    if(task.isSuccessful()) {
                        for(DocumentSnapshot snapshot : task.getResult()) {
                            ...
                            if(task.getResult().getDocuments().size() >= 1) {
                                ...
                                next = db.collection("실시간인기글").orderBy("write_time", Query.Direction.DESCENDING).startAfter(lastVisible).limit(20);
                            } else {
                                next = null;
                            }
                            ...
                            if(search_data.isEmpty()) {
                                ...
                                if(checkfirst) {
                                    recyclerView.setLayoutManager(new LinearLayoutManager(SearchPopularActivity.this));
                        ...
                        waiting.setVisibility(View.GONE);
                        nodata.setVisibility(View.VISIBLE);
                        Toast.makeText(SearchPopularActivity.this, "검색 실패", Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
    ...
    private class MySearchAdapter extends RecyclerView.Adapter {
        ...
        public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder viewHolder, final int i) {
            ...
            viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(SearchPopularActivity.this, ShowEachBoardActivity.class);
                    ActivityOptions options = ActivityOptions.makeCustomAnimation(SearchPopularActivity.this, R.anim.fromright, R.anim.toleft);

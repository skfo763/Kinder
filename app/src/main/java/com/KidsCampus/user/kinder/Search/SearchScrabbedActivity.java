...
public class SearchScrabbedActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_search_scrabbed);
        search = findViewById(R.id.searchscrabbed_search);
        recyclerView = findViewById(R.id.searchscrabbed_recyclerview);
        searchtext = findViewById(R.id.searchscrabbed_edittext);
        init = findViewById(R.id.searchscrabbed_init);
        nodata = findViewById(R.id.searchscrabbed_nodata);
        waiting = findViewById(R.id.searchscrabbed_waiting);
        progressBar = findViewById(R.id.searchscrabbed_progrssbar);
        ...
        search.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(searchtext.getText().toString().isEmpty() || searchtext.getText().toString().length() < 2) {
                    Toast.makeText(SearchScrabbedActivity.this, "검색어는 두 글자 이상 입력해주세요", Toast.LENGTH_SHORT).show();
                } else {
                    search_data.clear();
                    checkfirst = true;
            ...
            query.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                ...
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if(task.isSuccessful()) {
                        for(DocumentSnapshot snapshot : task.getResult()) {
                            ...
                            if(task.getResult().getDocuments().size() >= 1) {
                                ...
                                next = db.collection("사용자").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
                                        .collection("scrab").orderBy("scrab_time", Query.Direction.DESCENDING).startAfter(lastVisible).limit(20);
                            } else {
                                next = null;
                            }
                            ...
                            if(search_data.isEmpty()) {
                                ...
                                if(checkfirst) {
                                    recyclerView.setLayoutManager(new LinearLayoutManager(SearchScrabbedActivity.this));
                        ...
                        waiting.setVisibility(View.GONE);
                        nodata.setVisibility(View.VISIBLE);
                        Toast.makeText(SearchScrabbedActivity.this, "검색 실패", Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
    ...
    private class MySearchAdapter extends RecyclerView.Adapter {
        ...
        public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder viewHolder, final int i) {
            ...
            viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(SearchScrabbedActivity.this, ShowEachBoardActivity.class);
                    ActivityOptions options = ActivityOptions.makeCustomAnimation(SearchScrabbedActivity.this, R.anim.fromright, R.anim.toleft);
